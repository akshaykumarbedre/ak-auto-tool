...
| `custom_metadata` | object | Optional metadata attached at upload time. |

----------
---
  Title:
  docs: add complete REST API reference for ADK web server
  ...
  The existing REST API reference page (`docs/api-reference/rest/index.md`) only
  documented two endpoints (`/run` and `/run_sse`). This PR expands it to cover
  all 40 endpoints exposed by the ADK web server.
  ...
  - **Data models** — `Content`, `Event`, `Session`, `ArtifactVersion` with
    field-level descriptions and JSON examples

  All endpoint descriptions were derived directly from the source in
  `src/google/adk/cli/adk_web_server.py` and `src/google/adk/cli/fast_api.py`.
  The deprecated `POST /sessions/{session_id}` endpoint is included and marked
  as deprecated.

  ## Test plan

  - [ ] Verify rendered docs locally with `mkdocs serve`
  - [ ] Spot-check a running `adk web` server at `/docs` to confirm endpoint
    signatures match
  - [ ] Confirm all internal doc links resolve correctly

  ---
  Two things to do before you click "Create PR":

  1. Tag the maintainer — add this at the bottom of your description:
  cc @polong-lin @boyangsvl
  1. These are the two maintainers actively discussing this issue — tagging them gets faster review.                  
  2. Check your branch name — name your branch something like:
  docs/rest-api-reference                                                                                             
  2. Clean branch names signal you know what you're doing.